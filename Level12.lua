...
local function gotoDeathScreen()
  ...
  local function gotoLevel13(self, event)
      if(event.other.myName == "exit" or event.other.myName == "character") then
        if(event.phase == "began") then
          composer.setVariable( "livesCount", livesCount )
          composer.setVariable( "featherCount", featherCount )
          composer.setVariable( "nextScene", "Level13" )
...
local function movingGeese()
 local function goose3_1()
   goose3.xScale = 1
   transition.to(goose3,{ time = 1250, x = (goose3.x + 280), onComplete=goose3_2  })
 end

 goose3_2  = function ()
   goose3.xScale = -1
   transition.to(goose3,{ time = 1250, x = (goose3.x - 280), onComplete=goose3_1  })
 end
goose3_1()
end
 ...
 local function gooseFollow( event )
     ...
     if(character.x < goose2.x)then
     goose2.xScale=1
     transition.to( goose2, { time=3000, x=(character.x), y=(character.y), transition = easing.easeInOut  } )
     prevTime = system.getTimer()
   elseif(character.x > goose2.x)then
     goose2.xScale=-1
     transition.to( goose2, { time=3000, x=(character.x), y=(character.y), transition = easing.easeInOut  } )
     prevTime = system.getTimer()
   end
...
audio.setVolume( 0.1, { channel=4 } )
  ...
  mapData = require "objects.Screen12" -- load from lua export
  ...
  character.isFixedRotation = true
    ...
    hole.myName = "hole"
    hole.collision = triggerCollisionDeath
  ...
  hole:addEventListener("collision")
