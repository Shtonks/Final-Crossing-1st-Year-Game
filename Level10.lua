...
if gun==true then
 ...
 local function gooseFollow( event )
     if ( system.getTimer()-prevTime >= 1500 ) then
       if(character.x < goose.x)then
       goose.xScale=1
       transition.to( goose, { time=2000, x=(character.x), y=(character.y), transition = easing.easeInOut  } )
       prevTime = system.getTimer()
     elseif(character.x > goose.x)then
       goose.xScale=-1
       transition.to( goose, { time=2000, x=(character.x), y=(character.y), transition = easing.easeInOut  } )
       prevTime = system.getTimer()
     end
...
audio.setVolume( 0.1, { channel=4 } )
  ...
  mapData = require "objects.Screen10" -- load from lua export
  ...
  goose.myName = "goose"
  -- transition.to(goose,{ time = 5000, x = (goose.x - 700), iterations=0, transition = easing.continuousLoop  })
  goose.collision = triggerCollisionDeath

  exit = map:findObject("exit")
  exit.myName = "exit"

  exit.collision = gotoLevel13
